...
void adaptmesh(Mesh & Th,KN<R> un){
  ...
  R tol=0.;
  for (int k=0;k<Th.nt;k++) tol += nu[k];
  tol=tol/Th.nt;
  cout<<"tol="<<tol<<endl;
...
int main (int argc, char** argv){ 
  ...
  for(int adapt=0;adapt<3;adapt++){//RESOLUTION 3 FOIS
	...
	//___________calcul de  b1
	MatLap2d M1(Th,1.,0.,0.,0.,0); b1= M1*un;
	//__________calcul de b2
	...
	//_________calcul de b3 pour gamma robin
    ...
    //std::string baseName = getBaseName(cmsh);
