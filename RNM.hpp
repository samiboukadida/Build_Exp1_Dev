...
class ShapeOfArray{ protected:
  ...
  long myIndex(long k) const { K_throwassert( (k>=0) && ( (k <n) || !step) );
           return step*k;}
...
public:
  ...
  KN_(const KN_<R> & U,const SubArray & sa)  : ShapeOfArray(U,sa),v(U.v + U.myIndex(sa.start)) {}
  
  KN_ operator()(const SubArray & sa) const { return KN_(*this,sa);} // sub array 
  
  R & operator[](long i) const {return v[this->myIndex(i)];}
  R & operator()(long i) const {return v[this->myIndex(i)];}
  R & operator[](int i) const {return v[this->myIndex(i)];}
  R & operator()(int i) const {return v[this->myIndex(i)];}
...
class KNM_: public KN_<R> {
  ...
  long ij(const long i,const long j) const   
            { return shapei.myIndex(i)+shapej.myIndex(j);}
  long indexij(long i,long j)        const   
            { return this->myIndex(shapei.myIndex(i)+shapej.myIndex(j));}
  ...
  KN_<R> operator()(const char,long j    )  const   // une colonne j  ('.',j)
            { return KN_<R>(&this->v[this->myIndex(shapej.myIndex(j))],shapei*this->step);}
  KN_<R> operator()(long i    ,const char)  const   // une ligne i  (i,'.')
            { return KN_<R>(&this->v[this->myIndex(shapei.myIndex(i))],shapej*this->step);}
...
private:  
  KNM_& operator++() {this->v += next;return *this;} // ++U
  KNM_& operator--() {this->v -= next;return *this;} // ++U
  KNM_  operator++(int ){KNM_<R> old=*this;this->v = this->v +next;return old;} // U++
  KNM_  operator--(int ){KNM_<R> old=*this;this->v = this->v -next;return old;} // U--
...
//     shapek(k,n*m,n*m){};
  ...
  long ijk(const long i,const long j,const long k) const 
              { return shapei.myIndex(i)+shapej.myIndex(j)+shapek.myIndex(k);}
  long indexijk(long i,long j,long k) const 
              {return this->myIndex(shapei.myIndex(i)+shapej.myIndex(j)+shapek.myIndex(k));}
...
//       { if(unset()) set(new R[A1x.v.N()],A1x.v.N());KN_<R>::operator=(A1x);return *this;}
   KN& operator =(const typename VirtualMatrice<R>::plusAx & Ax)  
        { if(this->unset()) this->set(new R[Ax.x.N()],Ax.x.N());KN_<R>::operator=(Ax);return *this;}
...
//          { return (const KN<const_R>& ) *this;}
  void init(long nn) {this->n=nn;this->step=1;next=-1;this->v=new R[nn];}
  void destroy(){delete [] this->v; /*v=0;*/this->n=0;}
};
